...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, target_idx):
    ...
    auc_max = 0.
    auc_no_ext_max = 0.
    model_file  = os.path.join(args.model_dir, f'Dermalog_aug_{args.kernel_type}_best_fold{fold}.pth')
    model_file2 = os.path.join(args.model_dir, f'Dermalog_aug_{args.kernel_type}_best_no_ext_fold{fold}.pth')
    model_file3 = os.path.join(args.model_dir, f'Dermalog_aug_{args.kernel_type}_final_fold{fold}.pth')
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        print(content)
        with open(os.path.join(args.log_dir, f'log_Dermalog_aug_{args.kernel_type}.txt'), 'a') as appender:
