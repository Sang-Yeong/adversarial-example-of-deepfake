...
python AE_method_attack_img.py --kernel-type 5fold_b3_256_30ep --data-folder original_stone/ --enet-type tf_efficientnet_b3_ns --n-epochs 30
...
python AE_method_attack_img.py --kernel-type 5fold_b5_30ep --k-fold 5 --data-folder sampled_face/ --enet-type tf_efficientnet_b5_ns --n-epochs 2 --batch-size 32
...
def val_epoch(model, loader, target_idx, is_ext=None, n_test=1, get_output=False):
    ...
    TARGETS = []
    # with torch.no_grad():
    for i, (data, target, image_name) in enumerate(tqdm(loader)):
...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, target_idx, df_test):
    ...
    model_file_basic = os.path.join(args.model_dir, f'{args.kernel_type}_final_fold{fold}.pth')
    model_file_sys = os.path.join(args.model_dir, f'{args.kernel_type}_alb01_final_fold{fold}.pth')
    model_file_sbs = os.path.join(args.model_dir, f'{args.kernel_type}_best_fold{fold}.pth')

    # pretrained file이 있는 경우
    if os.path.isfile(model_file_sbs):
        model = ModelClass(
            args.enet_type,
    ...
    torch.save(model.state_dict(), model_file_basic)
